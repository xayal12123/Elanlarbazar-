...
// 1. SUPABASE BAĞLANTISI (Sənin real açarınla)
...
function AdCard({ title, price, city, image_url, is_vip }) {
  return (
    <div className={`relative bg-zinc-900 border ${is_vip ? 'border-green-500 shadow-[0_0_20px_rgba(34,197,94,0.2)]' : 'border-zinc-800'} rounded-2xl overflow-hidden transition-all hover:scale-[1.02]`}>
      {is_vip && (
        <div className="absolute top-3 right-3 bg-green-500 text-black text-[10px] font-black px-2 py-1 rounded-md uppercase z-10">
          VIP
        </div>
      )}
      <div className="h-40 bg-zinc-800 relative">
        {image_url ? (
          ...
          <div className="w-full h-full flex items-center justify-center text-zinc-700 font-mono text-[10px]">ŞƏKİL YOXDUR</div>
        )}
      </div>
      <div className="p-4">
  ...
  if (errorMsg) {
    return (
      <div className="min-h-screen bg-black flex items-center justify-center p-10">
        <div className="text-red-500 font-mono text-xs border border-red-900 p-5 rounded-xl bg-red-950/20">
          [ SİSTEM XƏTASI: {errorMsg} ]
        </div>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-black text-white">
        ...
        <div className="container mx-auto px-6 py-5 flex justify-between items-center">
          <div className="flex items-center gap-2">
            ...
            <h1 className="text-xl font-black italic uppercase tracking-tighter">
              ELANBAZARI <span className="text-green-500 underline decoration-2 offset-4">NOIR</span>
            </h1>
          </div>
          <button className="bg-white text-black px-6 py-2 rounded-full text-[10px] font-black uppercase tracking-widest hover:bg-green-500 transition-all">
            GİRİŞ
          </button>
        </div>
      </header>
      ...
      <main className="container mx-auto px-6 py-10">
        <div className="mb-10">
          ...
          <div className="h-1 w-20 bg-green-500 mt-4 shadow-[0_0_15px_#22c55e]"></div>
        </div>

        {loading ? (
          <div className="flex flex-col items-center justify-center py-20 gap-4">
            <div className="w-10 h-10 border-4 border-green-500 border-t-transparent rounded-full animate-spin"></div>
            <p className="text-zinc-500 font-mono text-[10px] uppercase tracking-[0.4em]">[ BAZA SKAN EDİLİR ]</p>
          </div>
        ) : elanlar.length === 0 ? (
